...
void init_graphics() {
  ...
  // to load whichever video driver supports the flags we're passing
  //
  // Flags:
  ...
  // screen) to be synchornized with the monitor's refresh rate
  render =
  ...
  // Load font
  gFont = TTF_OpenFont("Roboto-Regular.ttf", 40);
...
void setRenderChanged() { render_changed = true; }

void preRender() { SDL_SetRenderTarget(render, display); }
